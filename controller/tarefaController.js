...
class tarefaController{
  ...
  viewCreate(req, res) {
    const tarefamemberlist = tarefaModel.readMembers();
    return tarefamemberlist
      .then((result) =>
        result.length == 0
        ? res.status(404).render("./tarefa/tarefa_create", { title: "Adicionar uma nova tarefa!", results: result, search:''})
        : res.status(200).render("./tarefa/tarefa_create", { title: "Adicionar uma nova tarefa!", results: result, search:''})
        )
